...
La \hyperref[fig:clasificacion_equipos_resumen]{Figura~\ref*{fig:clasificacion_equipos_resumen}} resume la organización modular de esta fase. Como puede observarse, no se trata de un proceso estrictamente secuencial: el flujo principal consulta el módulo de extracción cromática, alimenta una memoria online de colores de referencia y combina esa información con criterios auxiliares para emitir la decisión final conjunta de clase y equipo.
...
\node[block] (input)
{Entrada\\
detecciones + clase YOLO\\
+ posición en campo};

\node[block, right=1cm of input] (extract)
{Extracción del\\
color LAB de camiseta};

% -------------------------------------------------
% ShirtDetector
% -------------------------------------------------
\node[smallblock, above=1.75cm of extract, xshift=-0.45cm] (crop)
{Recorte del 50\%\\
superior del \textit{bbox}};

\node[smallblock, above=0.42cm of crop] (kmeansshirt)
{KMeans sobre\\
el crop de detección};

\node[container, fit=(crop)(kmeansshirt)] (shirtbox) {};
\node[boxtitle, anchor=south west] at ($(shirtbox.north west)+(0.15,0.02)$)
{ShirtDetector};

\draw[arrow] (crop.north) -- (kmeansshirt.south);
\draw[darrow] (shirtbox.south) -- (extract.north);

% -------------------------------------------------
% Modelo cromático online
% -------------------------------------------------
\coordinate (memaxis) at ($(extract.center)+(7.55cm,2.35cm)$);

\node[smallblock] (fieldbucket)
at ($(memaxis)+(-2.55cm,3.25cm)$)
{Muestras acumuladas\\
color LAB jugadores};

\node[smallblock] (refbucket)
at ($(memaxis)+(2.55cm,3.25cm)$)
{Muestras acumuladas\\
color LAB árbitros};

\node[smallblock] (teamkmeans)
at ($(memaxis)+(-2.55cm,1.62cm)$)
{Referencias colores equipo\\
KMeans ($k=2$)};

\node[smallblock] (trimmed)
at ($(memaxis)+(2.55cm,1.62cm)$)
{Referencia color árbitro\\
media LAB de muestras};

\node[wideblock] (robust)
at ($(memaxis)+(0,0.25cm)$)
{Referencias cromáticas, IQR\\
y umbrales de pertenencia};

% -------------------------------------------------
% Decisor cromático
% Mismo eje horizontal que input y extract
% -------------------------------------------------
\node[decision] (decision)
at ($(robust.center |- extract.center)$)
{Fusión y\\
decisión\\
equipo/relabel};
...
% Mismo eje horizontal que input, extract y decision
% -------------------------------------------------
\coordinate (auxcenter) at ($(decision.center)+(6.10cm,0)$);

\node[auxblock] (gates)
at ($(auxcenter)+(0,0.52cm)$)
{Gates posicionales\\
banda, centro y portero};

\node[auxblock] (rules)
at ($(auxcenter)+(0,-0.52cm)$)
{Reglas de decisión\\
color cercano, outlier y relabel};

\node[container, fit=(gates)(rules)] (auxbox) {};
\node[boxtitle, anchor=south west] at ($(auxbox.north west)+(0.15,0.02)$)
{Criterios auxiliares};

% -------------------------------------------------
% Salida
% -------------------------------------------------
\node[block, below=1.45cm of decision] (output)
{Salida\\
\texttt{class}, \texttt{team}, \texttt{shirt\_color},\\
\texttt{distances}, \texttt{bbox\_size}};

% -------------------------------------------------
% Flechas del flujo principal
% -------------------------------------------------
\draw[arrow] (input) -- (extract);
\draw[arrow] (extract) -- (decision);
\draw[arrow] (decision) -- (output);

% Criterios auxiliares -> decisor cromático
\draw[darrow] (auxbox.west) -- (decision.east);

% -------------------------------------------------
% Coordenadas auxiliares del bus superior
% -------------------------------------------------
\coordinate (bustopy) at ($(fieldbucket.north)+(0,0.48cm)$);
\coordinate (feedrise) at ($(extract.north east |- bustopy)$);
\coordinate (busleft) at ($(fieldbucket.north |- bustopy)$);
\coordinate (busright) at ($(refbucket.north |- bustopy)$);

\node[inner sep=0pt] (feedrisenode) at (feedrise) {};
\node[inner sep=0pt] (busleftnode) at (busleft) {};
\node[inner sep=0pt] (busrightnode) at (busright) {};

\node[container, fit=(fieldbucket)(refbucket)(teamkmeans)(trimmed)(robust)(feedrisenode)(busleftnode)(busrightnode)] (membox) {};
\node[boxtitle, anchor=south west] at ($(membox.north west)+(0.15,0.02)$)
{Memoria de colores referencia de los dos equipos y árbitro};

% -------------------------------------------------
% Flechas del modelo cromático
% -------------------------------------------------

% Extracción -> bus superior -> muestras acumuladas
\draw[dashed, thick] (extract.north east) -- (feedrise) -- (busleft) -- (busright);
\draw[darrow] (busleft) -- (fieldbucket.north);
\draw[darrow] (busright) -- (refbucket.north);

% Muestras -> estimación de referencias
\draw[arrow] (fieldbucket.south) -- (teamkmeans.north);
\draw[arrow] (refbucket.south) -- (trimmed.north);

% Referencias de equipo y árbitro -> referencias robustas
\coordinate (mergekmtrim) at ($(robust.center |- teamkmeans.center)$);

\draw[thick] (teamkmeans.east) -- (mergekmtrim);
\draw[thick] (trimmed.west) -- (mergekmtrim);
\draw[arrow] (mergekmtrim) -- (robust.north);

% Referencias robustas -> decisor cromático
\draw[arrow] (robust.south) -- (decision.north);

\end{tikzpicture}
}%
\caption[Resumen modular de la clasificación de equipos]{Resumen modular de la fase de clasificación de equipos. El flujo principal parte de las detecciones, extrae el color LAB de la camiseta y combina esa muestra en un decisor multicriterio.}
\label{fig:clasificacion_equipos_resumen}
\end{figure}

A continuación se describe con más detalle cada uno de estos bloques, comenzando por la extracción del color representativo de la camiseta para cada detección.
...
En la \hyperref[fig:team-color-clusters-lab]{Figura~\ref*{fig:team-color-clusters-lab}} se puede ver cómo queda el estado de un partido tras una ejecución. El sistema es capaz de definir clusters bastante acertados para los colores de cada grupo de detecciones. En concreto, se observa el cluster del Equipo 1, de color verde; el cluster del Equipo 2, de color blanco; y el cluster de árbitros, de color negro.
...
La configuración operativa del módulo se resume en la \hyperref[tab:clasificacion_equipos]{Tabla~\ref*{tab:clasificacion_equipos}}, donde se describen los hiperparámetros del bloque de clasificación de equipos.
...
Bootstrap por clase &
\texttt{min\_samples\_player} &
\texttt{60} &
Número mínimo de muestras acumuladas antes de consolidar los centros cromáticos de jugadores. \\

&
\texttt{min\_samples\_referee} &
\texttt{15} &
Número mínimo de muestras acumuladas antes de consolidar el centro cromático del árbitro. \\

\midrule

Validación mínima de clusters &
\texttt{min\_cluster\_size} &
\texttt{15} &
Rechaza particiones demasiado desbalanceadas al ajustar los clusters de jugadores. \\

\midrule

Confianza mínima de muestreo &
\texttt{min\_conf\_player} &
\texttt{0.8} &
Filtra las detecciones de jugadores que pueden entrar en los buckets de color. \\

&
\texttt{min\_conf\_referee} &
\texttt{0.7} &
Filtra las detecciones de árbitro que pueden entrar en su bucket cromático. \\

\midrule

Memoria de muestras &
\texttt{max\_samples\_class} &
\texttt{500} &
Limita el número de muestras LAB almacenadas por clase para el ajuste online. \\

\midrule

Bootstrap de árbitros &
\texttt{ref\_bootstrap\_margin} &
\texttt{10} &
Controla cuándo se puede consolidar el bucket cromático del árbitro. \\

&
\texttt{ref\_upper\_bound\_cap} &
\texttt{20.0} &
Limita el radio de pertenencia del árbitro en el espacio LAB. \\

\bottomrule
\end{tabularx}

\caption[Hiperparámetros de clasificación de equipos]{
Hiperparámetros principales de la fase de clasificación de equipos.}
\label{tab:clasificacion_equipos}
\end{table}


Como ya se ha comentado, el valor final de los hiperparámetros se decide tras realizar muchas pruebas visuales sobre vídeo de referencia. Este proceso se describe en la \hyperref[sec:evaluacion-heuristica-depuracion]{Sección~\ref*{sec:evaluacion-heuristica-depuracion}}.

Finalmente, cabe destacar que esta fase fue uno de los cuellos de botella iniciales del pipeline. En una primera versión, el clustering de cada camiseta se realizaba en CPU con \texttt{scikit-learn}, lo que incrementaba el tiempo de procesamiento por frame. Para reducir esta latencia se implementó una versión batched del KMeans con aceleración en GPU cuando está disponible (basada en \texttt{CuPy}), manteniendo \texttt{scikit-learn} como alternativa en CPU. En nuestras pruebas, esta optimización redujo el tiempo aproximado de identificación de camisetas y asignación de equipo de 1.5 s/frame a 0.05 s/frame, lo que supone una aceleración de aproximadamente $\times30$.