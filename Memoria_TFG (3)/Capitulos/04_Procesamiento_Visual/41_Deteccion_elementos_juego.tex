...
El valor final de estos hiperparametros se fijo mediante depuracion iterativa y pruebas visuales sobre video de referencia; este proceso se describe en la \hyperref[sec:evaluacion-heuristica-depuracion]{Seccion~\ref*{sec:evaluacion-heuristica-depuracion}}.
...
La decisión final del detector no debe basarse únicamente en estas métricas de detección aislada. El sistema completo depende también del comportamiento del tracking, de la estabilidad temporal de las identidades y de cómo se comporta el modelo sobre vídeos reales, donde aparecen compresión, movimiento de cámara, cambios de plano y oclusiones. Por este motivo, aunque esta comparación favorece al modelo sintético+DFL, la decisión final se deja abierta hasta completar las pruebas de tracking sobre vídeo real. Esta evaluación final se describe al final de este capítulo, en la \hyperref[sec:evaluacion-heuristica-depuracion]{Seccion~\ref*{sec:evaluacion-heuristica-depuracion}}

En la \hyperref[fig:detect_frame]{Figura~\ref*{fig:detect_frame}} se muestra un ejemplo visual de las predicciones del modelo secuencial SoccerSynth+DFL sobre un frame del vídeo de prueba. Se pueden observar las cajas delimitadoras y las clases asignadas a cada detección, lo que permite comprobar la calidad visual de las predicciones en un escenario realista. El video del resultado completo se puede encontrar en \url{https://github.com/hectorgarciaa/narrador-futbol/blob/main/output/detect/best/20260516_172142/detections.mp4}
\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\textwidth]{Imagenes/Propias/detect_frame.png}
    \caption[Resultado visual de la fase de detección]{
    Resultado visual de la fase de detección sobre un frame del vídeo de prueba. Se muestran las predicciones del modelo secuencial SoccerSynth+DFL, con sus respectivas clases y bounding boxes.
    }
    \label{fig:detect_frame}
\end{figure}