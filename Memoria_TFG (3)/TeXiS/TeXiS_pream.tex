...
\ifpdf
   \RequirePackage[pdfborder={0 0 0},hidelinks,hyperindex,pdfpagelabels,hypertexnames=false]{hyperref}
...
% Los audios deben estar dentro del proyecto LaTeX, en audios/.
% El segundo argumento puede ser el nombre limpio del fichero:
%   \audiobutton{Muestra A2}{intro_multilingual_v2.wav}
% o una ruta antigua como ../audios/intro_multilingual_v2.wav; el macro
% descarta el directorio y busca siempre audios/<nombre>.
% Visualmente se dibuja un mini-reproductor con TikZ, sin depender de PNG.
\newif\ifmediaNineAvailable
\IfFileExists{media9.sty}{%
  \usepackage{media9}
  \addmediapath{Audios/}
  \mediaNineAvailabletrue
}{}

\makeatletter
\newcommand{\audioplayerbox}[1]{%
    \begin{tikzpicture}[baseline=-0.55ex]
        ...
        \node[font=\scriptsize] at (-2.25,0) {$\blacktriangleright$};
        \node[font=\scriptsize,anchor=west] at (-1.95,0) {#1};
        \draw[line width=1.6pt,line cap=round,black!55] (0.05,0) -- (1.05,0);
        \node[font=\scriptsize] at (1.42,0) {$\bullet$};
        \node[font=\scriptsize] at (1.72,0) {$\vdots$};
    \end{tikzpicture}%
}
\newcommand{\audiobutton}[2]{%
    \begingroup
    \filename@parse{#2}%
    \edef\audio@file{Audios/\filename@base.\filename@ext}%
    \edef\audio@target{run:\audio@file}%
    \href{\audio@target}{\audioplayerbox{#1}}%
    \endgroup
}
\newcommand{\audioembeddedbutton}[2]{%
    \begingroup
    \filename@parse{#2}%
    \edef\audio@file{Audios/\filename@base.\filename@ext}%
    \sound[inlinesound,samplingrate=24000,bitspersample=16]{%
        \audioplayerbox{#1}%
    }{\audio@file}%
    \endgroup
}
\newcommand{\audiomediaNinebutton}[2]{%
    \begingroup
    \filename@parse{#2}%
    \edef\audio@mpthree{\filename@base.mp3}%
    \ifmediaNineAvailable
        \includemedia[
            width=5.2cm,
            height=0.78cm,
            activate=onclick,
            transparent,
            addresource=\audio@mpthree,
            flashvars={source=\audio@mpthree&autoPlay=true}
        ]{\audioplayerbox{#1}}{APlayer.swf}%
    \else
        \audiobutton{#1}{#2}%
    \fi
    \endgroup
...
\newcommand{\audiolink}[2]{%
\fbox{\href{#2}{\underline{#1}}}%
}