...
Si el tracking online de roles está activo, `track.py` exporta además `output/tracks_json/tracker/<video_sanitizado>_frame_role_predictions.csv`, `output/tracks_json/tracker/<video_sanitizado>_player_role_summary.csv` y `output/tracks_json/tracker/<video_sanitizado>_greedy_role_diagnostics.csv`. El CSV por frame conserva la predicción cruda del modelo antes del congelado estable, y el CSV del greedy guarda las métricas acumuladas que usó la estabilización para elegir slot por slot.
