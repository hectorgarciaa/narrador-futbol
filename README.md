...
Si `tracking.special_seed_role_team_assignment_enabled=true`, el tracking principal ejecuta además el modelo de `position_role` frame a frame durante el tracking para los jugadores normales y usa a los defensas detectados en ese frame para asignar equipo a los IDs reservados `1-2` por defensa más cercano. Esos dos IDs no entran al Set Transformer: se etiquetan manualmente como `POR`. Además, si defines `tracking.expected_roles_by_team`, la inferencia posicional online ajusta sus plazas a ese once esperado y la asignación estable final se congela respetando esas posiciones por equipo. En ese modo, `predicted_role` pasa a ser la plaza táctica asignada y se conservan `predicted_role_unconstrained` y `matched_model_role` para inspeccionar la salida libre del modelo. Esos IDs no usan color de camiseta para recuperar identidad ni para fijar su equipo.
