...
- El flujo rolling live de acciones ya no depende de un postproceso offline en disco para `shot` y `robo`: `football_ai/actions/rolling.py` refina cada bloque emitido en memoria, manteniendo estado causal de posesión y dirección de ataque para promocionar algunos `kick` a `shot` y reinterpretar recuperaciones como `robo` antes de que la fase de comentarios las consuma.
...
En el postproceso online de roles, `display_role_slot` pasa a ser la única fuente de verdad del slot estable por segmento. El resto de campos de rol estable duplicados (`predicted_role`, `expected_role_slot`) ya no se escriben en `tracks.json` para evitar inconsistencias; consumidores internos (comentarios, adaptador PathCRF) leen `display_role_slot` como prioridad.
La segunda pasada de Húngaro de posiciones (Tier 2) orienta ya sus anclas con la misma convención que Tier 1: si un equipo ataca hacia la izquierda, se hace flip de la grid al comparar contra `pitch_layout`, y la desambiguación de slots repetidos (`*_IZQ`, `*_DCHO`) usa el eje lateral orientado en vez de la coordenada absoluta del campo.
...
- en ese mini-mapa incrustado ya no se pintan overlays de `frame/EDGE/EVENT` ni etiquetas extendidas con `player_name`/`role`: esas etiquetas se mantienen en las `bbox` del broadcast.
- el bloque `EVENT` del HUD se muestra junto a `frame` y `EDGE` en la esquina superior izquierda y usa IDs de slot PathCRF (`home_*`, `away_*`, `referee_*`) cuando existe mapeo desde IDs canónicos/raw.
...
- en el flujo live de `scripts/track.py`, el bloque rolling también refina ya en memoria `kick/control/out` a `shot/robo/...` sin escribir parquets intermedios ni reutilizar el pipeline offline de comentarios.
...
tracking:
  ...
  segment_temporal_continuity_enabled: true  # penaliza cambios de slot dentro del mismo segmento
  segment_temporal_same_base_penalty: 0.15   # cambio entre slots que comparten la misma familia base (p.ej. DC_IZQ <-> DC_DCHO)
  segment_temporal_same_line_penalty: 0.35   # cambio dentro de la misma linea tactica
  segment_temporal_cross_line_penalty: 1.25  # salto entre defensa/medio/ataque
