...
- La normalización de `Detector` ya deja pasar solo las clases soportadas (`player`, `goalkeeper`, `referee`, `ball` y aliases equivalentes). Después, el filtrado post-homografía vive en `football_ai/filtering/` con una única función pública reutilizada por `Tracker` y `scripts/homography.py`: invalida `field_position_m` cuando la homografía no es fiable para tracking y, si la proyección es usable, descarta detecciones fuera del terreno con tolerancia lateral para linieres y excepción por solape con tracks activos.
...
Si el tracking online de roles está activo, `track.py` exporta además los CSV y PNG canónicos en `output/tracker/<video_sanitizado>_role_artifacts/`, incluyendo `<video_sanitizado>_frame_role_predictions.csv`, `<video_sanitizado>_player_role_summary.csv` y `<video_sanitizado>_greedy_role_diagnostics.csv`. La inferencia online usa el Set Transformer frame a frame con Hungarian por equipo contra `tracking.expected_roles_by_team`; los canónicos ausentes pueden reaparecer como contexto interpolado durante unos pocos frames para mejorar el vecindario del modelo y también participan en ese Hungarian del frame, aunque sigan sin contaminar el histórico semántico ni los segmentos. Además, el histórico por `canonical_id` ya no se trata como una única bolsa: se segmenta dinámicamente cuando hay un salto espacial fuerte y, además, cambia el rol reciente o la capa canónica relinka el ID. Esos cortes se reflejan en `tracks.json` con metadatos como `identity_segment_id`, `identity_reset_reason`, `canonical_assignment_mode` y `source_raw_tracker_id`. El CSV de `greedy_role_diagnostics` se mantiene por compatibilidad con la lógica antigua de congelado/diagnóstico; en el flujo nuevo puede quedar vacío si no se activa esa vía legacy. También deja una copia de compatibilidad en `output/tracks_json/tracker/`.
