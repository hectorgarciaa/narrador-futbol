...
Si `tracking.special_seed_role_team_assignment_enabled=true`, el tracking principal ejecuta además el modelo de `position_role` frame a frame durante el tracking y usa a los defensas detectados en ese frame para asignar equipo a esos IDs reservados por defensa más cercano. Después hace un segundo pase de roles en ese mismo frame para que esos IDs especiales también queden anotados con `predicted_role` cuando ya son detecciones reales. Esos IDs no usan color de camiseta para recuperar identidad ni para fijar su equipo.
