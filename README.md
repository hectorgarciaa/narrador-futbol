...
- Una vez elegidos los hiperparámetros del Set Transformer, el entrenamiento final puede relanzarse con `train+val` y evaluación única en `test` mediante `python -m football_ai.positions.set_transformer_grid_search --final-train-val --rebuild-from-base-table`; los artefactos se guardan en `models/positions/final_train_val/`.
