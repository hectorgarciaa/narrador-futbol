#!/usr/bin/env python3
"""
Script para detectar objetos con el modelo YOLO base (sin fine-tuning)
...
def detect_on_video(video_key, output_dir="output/deteccion_base"):
    """
    Procesa un video con detecciones YOLO y guarda resultados.
    ...
    model_path = config.get_path('paths', 'models', 'yolo_v11_m')
    
    # Resolver rutas
    output_dir = Path(config.get_path('paths', 'output', 'base', create_if_missing=True)) / "deteccion_base"
    output_dir.mkdir(parents=True, exist_ok=True)
    
    print(f"Detectando con modelo base YOLO")
    ...
    while True:
        ...
        # Filtrar solo person boxes antes de dibujar
        person_indices = []
        for idx, box in enumerate(results[0].boxes):
            if model.names[int(box.cls)] == 'person':
                person_indices.append(idx)
        
        # Crear resultado filtrado para dibujar
        if person_indices:
            filtered_boxes = results[0].boxes[person_indices]
            # Dibujar manualmente sobre el frame
            annotated_frame = frame.copy()
            for box_idx in person_indices:
                box = results[0].boxes[box_idx]
                x1, y1, x2, y2 = map(int, box.xyxy[0])
                conf = float(box.conf)
                # Dibujar bbox (verde para person)
                cv2.rectangle(annotated_frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
                # Dibujar label con confianza
        ...
        # Almacenar detecciones para JSON (solo person)
        frame_detections = {
            'frame': frame_count,
            'detections': []
        }
        ...
        for box in results[0].boxes:
            ...
            # Filtrar solo detecciones de person
