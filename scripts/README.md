...
**Nota IDs especiales 1-2:** si `tracking.special_seed_role_team_assignment_enabled=true`, `track.py` ejecuta el modelo de roles posicionales frame a frame durante el tracking y usa el defensa más cercano de ese frame para asignar equipo a esos IDs reservados. Después relanza la inferencia de roles para incluir a esos IDs cuando ya son detecciones reales y, si reaparecen con un `raw_tracker_id` previamente ligado a otro canónico, pueden reclamarlo por compatibilidad geométrica. Para ellos no se usa color de camiseta como criterio de recuperación.
