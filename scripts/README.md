...
**Nota IDs especiales 1-2:** si `tracking.special_seed_role_team_assignment_enabled=true`, `track.py` ejecuta el modelo de roles posicionales frame a frame durante el tracking solo para los jugadores normales y usa el defensa más cercano de ese frame para asignar equipo a esos IDs reservados. Los IDs `1-2` no entran al modelo y se fuerzan manualmente a `POR`. Si `tracking.expected_roles_by_team` está definido, esa misma inferencia online restringe las posiciones posibles de cada equipo con Hungarian usando el once esperado; el `predicted_role` final pasa a ser la plaza táctica asignada y `matched_model_role` conserva la label libre del modelo. Si reaparecen con un `raw_tracker_id` previamente ligado a otro canónico, solo pueden reclamarlo cuando ese canónico ya no estaba realmente activo y la geometría les favorece. Para ellos no se usa color de camiseta como criterio de recuperación.
