...
**Nota anti-ID-switch:** `track.py` aplica gate estadístico (`motion_std_*`) y reglas estrictas de reasignación desde `config.yaml`; para `player/goalkeeper`, cuando hay homografía válida la continuidad y la reasignación se basan en distancia en campo, y cuando no la hay el sistema cae a píxeles con extrapolación de movimiento.
**Nota jugadores fuera de plano:** si `canonical.reserve_penalty_spot_seed_players=true`, el tracker reserva dos IDs canónicos sintéticos en los puntos de penalti. Esos seeds no entran en `TeamDetector`, pero sí pueden ser heredados por una detección real compatible cuando el jugador aparece en pantalla. Hasta entonces también se escriben en el JSON como `synthetic_seed=true`.
**Nota IDs especiales 1-2:** si `positions.special_seed_role_team_assignment_enabled=true`, `track.py` ejecuta el modelo de roles posicionales frame a frame durante el tracking solo para los jugadores normales y usa el defensa más cercano de ese frame para asignar equipo a esos IDs reservados. Los IDs `1-2` no entran al modelo y se fuerzan manualmente a `POR`. Si `positions.expected_roles_by_team` está definido, el flujo online usa dos pasadas configurables por separado con `positions.frame_expected_roles_assignment_method` y `positions.segment_expected_roles_assignment_method`; cada una puede usar `hungarian` o `ratio_priority`. La primera genera el voto limpio por frame y la segunda resuelve el slot definitivo usando la mayoría acumulada de cada segmento activo. Si el tracking detecta un relink fuerte o un salto estructural, abre un segmento nuevo para ese `canonical_id`.
**Salidas extra de roles:** cuando esa lógica online está activa, `track.py` escribe además los CSV y PNG de roles dentro de `output/tracker/<video>_role_artifacts/`. Ahí quedan `<video>_frame_role_predictions.csv`, `<video>_player_role_summary.csv` y `<video>_greedy_role_diagnostics.csv`; este último mantiene el nombre legacy por compatibilidad, pero ahora actúa como CSV general de diagnóstico de asignación (`hungarian` o `ratio_priority`). También deja una copia de compatibilidad en `output/tracks_json/tracker/`.
...
**Nota balón:** además del matching normal, `track.py` filtra el balón con restricciones específicas de trayectoria esperada y tamaño (configuradas en `canonical.ball.*`) para rechazar detecciones que se teletransportan o cambian de escala sin plausibilidad física, intentando mantener la cobertura original del detector. El umbral mínimo de confianza del balón se toma de `canonical.ball.ball_min_conf` en `config.yaml` y puede bajarse más que el general porque el filtro posterior ya elimina candidatas imposibles. Si el balón sale por un borde de la imagen, la búsqueda posterior queda anclada a ese límite; solo tras `canonical.ball.ball_max_reassign_lost_frames` frames sin detección se permite redetección libre por confianza máxima. En el resumen final añade también `ball_coverage`.
