...
**Nota IDs especiales 1-2:** si `tracking.special_seed_role_team_assignment_enabled=true`, `track.py` ejecuta el modelo de roles posicionales frame a frame durante el tracking solo para los jugadores normales y usa el defensa más cercano de ese frame para asignar equipo a esos IDs reservados. Los IDs `1-2` no entran al modelo y se fuerzan manualmente a `POR`. Si reaparecen con un `raw_tracker_id` previamente ligado a otro canónico, solo pueden reclamarlo cuando ese canónico ya no estaba realmente activo y la geometría les favorece. Para todos los IDs visibles, el role queda congelado usando sus primeras observaciones (`tracking.role_stabilization_*`) y la posición estable se resuelve con una asignación única por equipo, evitando dos `predicted_role` estables iguales en el mismo equipo. Para ellos no se usa color de camiseta como criterio de recuperación.
