...
class Drawer:
    ...
    def _resolve_debug_detection_class_name(self, det):
        for key in ("class_name", "class_tracker", "class_td", "class_relabel", "class_team_detector", "class_yolo"):
            value = det.get(key)
            if value:
                return value
        return ""
    ...
    def draw_detection(
        ...
        elif constrained_expected_role and expected_role_slot:
            info_lines.append(f"role: {_format_role_overlay_label(expected_role_slot)}")
        elif segment_majority_expected_role_slot:
            info_lines.append(
