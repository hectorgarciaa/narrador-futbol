...
4. **ByteTrack** (`ByteTrack.update_with_detections`): asocia las detecciones a tracks con IDs persistentes entre frames. Usa la etiqueta de equipo como penalización adicional, combina doble señal de clase por detección (clase YOLO original + clase reetiquetada por `TeamDetector`) para permitir remapeos controlados cuando discrepan, añade consenso temporal de clase por track y, para `player`/`goalkeeper`, incorpora distancia en el campo 2D al coste de asociación. Para reducir duplicados, cualquier track `unconfirmed` se descarta si solapa mínimamente (`IoU>0`) con un track activo, no se permiten solapes entre `unconfirmed` previos, y los nuevos candidatos tampoco pueden solaparse con `unconfirmed` previos ni entre sí: ante cualquier solape se conserva solo el candidato de mayor confianza. Frente a tracks ya activos, ese filtro de nacimiento usa ahora `bytetracker.new_track_active_overlap_iou` (por ejemplo `0.25`) en vez de solape cero absoluto. En la capa canónica, si ByteTrack mantiene el mismo `raw_tracker_id`, ese vínculo se conserva directamente siempre que pase un gate de continuidad; solo si falla esa continuidad la detección vuelve a competir por otro ID canónico.
