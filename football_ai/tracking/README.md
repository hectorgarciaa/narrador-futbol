...
1. **Detección YOLO** (`Detector.detect`): genera las detecciones brutas del frame y normaliza `detections.names` a las clases canónicas del proyecto (`player`, `goalkeeper`, `referee`, `ball`) antes de que entren en tracking.
...
3. **PnLCalibFieldProjector + gates de relabel a árbitro/portero**: calibra el campo en ese frame y proyecta `player` y `goalkeeper` a coordenadas métricas `[x_m, y_m]` sobre el césped. Si la solución base sale floja, el proyector reintenta con thresholds más bajos de forma progresiva, evalúa cada intento con un score de validación basado solo en `geometry_fit`, `support_quality` y `coverage_quality`, y se queda con el mejor. Si ningún intento alcanza calidad suficiente, la homografía se rechaza en ese frame y el pipeline devuelve `field_position_m = null`.
...
6. **Lógica especial para los IDs reservados y roles online**: esos dos IDs quedan reservados a porteros. Solo detecciones cuya clase resuelta sea `goalkeeper` pueden ocupar los IDs 1 y 2, tanto en la capa canónica como al absorber sobre los seeds especiales. Su equipo se asigna frame a frame durante el propio tracking con el modelo de roles posicionales y el defensa más cercano. Como ya se consideran porteros conocidos, no entran al Set Transformer y se etiquetan manualmente como `POR`. Para el resto de jugadores, el frame hook online ejecuta el Set Transformer en cada frame con Hungarian por equipo contra `tracking.expected_roles_by_team`. Los canónicos ausentes pueden entrar unos pocos frames como contexto interpolado para que el modelo vea un once más completo y, además, esas filas interpoladas también participan en el Hungarian del frame para reservar slots plausibles aunque el jugador no sea visible en imagen. Aun así, las filas interpoladas no actualizan el histórico del jugador ni los segmentos semánticos: solo sirven para contexto y para la asignación táctica instantánea del frame. La memoria táctica ya no se acumula como una sola bolsa por `canonical_id`: se divide en segmentos y, si hay un salto espacial fuerte junto con un cambio claro de rol reciente o una reasignación/reabsorción canónica (`canonical_relinked`), el segmento anterior se cierra y el histórico nuevo arranca limpio. Así, el JSON final conserva tanto el rol crudo del modelo (`predicted_role_unconstrained`) como la plaza esperada de ese frame (`expected_role_slot`) y la segmentación semántica (`identity_segment_id`, `identity_reset_reason`).
...
- `max_tracks_per_class` (reparto actual recomendado: `goalkeeper=2`, `player=20`, `referee=3`, `ball=1`)
