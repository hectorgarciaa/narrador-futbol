...
6. **Lógica especial para los IDs reservados**: esos dos IDs no exigen coincidencia `player/goalkeeper` para recuperar una detección real y no fijan equipo por color durante el tracking. Su equipo se asigna frame a frame durante el propio tracking con el modelo de roles posicionales y el defensa más cercano. Como ya se consideran porteros conocidos, no entran al Set Transformer y se etiquetan manualmente como `POR`. Si defines `tracking.expected_roles_by_team`, ese once esperado se usa solo al congelar el `role` estable, no para imponer una plaza táctica distinta en cada frame. El congelado se decide con la evidencia acumulada del jugador durante la ventana de estabilización y luego sí se asigna una plaza esperada por equipo. Si una detección reaparece con un `raw_tracker_id` ya arrastrando otro canónico, los IDs especiales solo pueden reclamarla si ese canónico no estaba realmente activo y además la geometría favorece al ID especial.
