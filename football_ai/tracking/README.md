...
6. **Lógica especial para los IDs reservados**: esos dos IDs no exigen coincidencia `player/goalkeeper` para recuperar una detección real y no fijan equipo por color durante el tracking. Su equipo se asigna frame a frame durante el propio tracking con el modelo de roles posicionales y el defensa más cercano. Si una detección reaparece con un `raw_tracker_id` ya arrastrando otro canónico, los IDs especiales todavía pueden reclamarla si la geometría es compatible. Cuando el equipo ya está resuelto, el tracker hace un segundo pase de roles en ese mismo frame para escribir también su `predicted_role`.
