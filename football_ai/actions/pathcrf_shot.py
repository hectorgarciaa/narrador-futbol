...
def _has_gk_intervention(pred_events: pd.DataFrame, gk_map: dict[object, dict[str, str]]) -> pd.Series:
    ...
    next_goal_x = np.where(
        pred_events["next_team_id"] == "home",
        0.0,
        PITCH_LENGTH_M,
    )
...
def _calculate_speed_score(pred_events: pd.DataFrame) -> pd.Series:
    thresholds = np.where(
        ...
        SPEED_THRESHOLD_FAR_BASE_MPS
        + ((pred_events["dist_to_goal"] - SPEED_DISTANCE_BOUNDARY_M) * SPEED_THRESHOLD_FAR_SLOPE),
    )
