...
- `online_semantics.py`: aplica en memoria el refinado semántico del flujo rolling live. Parte de los eventos temporales `kick/control/out`, mantiene estado causal de posesión y dirección de ataque por equipo, promociona algunos `kick` a `shot` con reglas geométricas sobre posiciones de tracking y convierte recuperaciones (`kick` al rival o `control` tras posesión contraria) en `robo`;
- `pathcrf_commentary.py`: invierte `pathcrf_id -> track_id` con `person_slot_assignments`, recupera nombre/equipo/posición desde `tracks.json` y genera un JSON rico listo para pasar después a Gemma. También añade posición en metros, dirección de ataque estimada, `field_zone_key` (`iniciacion`, `creacion`, `finalizacion`) y `commentary_priority` para que la narración pese más las acciones cercanas a portería. Antes de marcar un evento como narrable mantiene un poseedor lógico: un `pase` aceptado mueve la posesión al receptor, un `control` debe coincidir con el poseedor, los pases desde alguien que ya no tiene el balón se saltan con `skip_reason=possession_actor_mismatch` y los pases a un jugador del rival se convierten en `robo` del receptor.

El JSON de comentarios conserva tanto el evento semántico enriquecido como un subpayload `commentary_event` listo para la fase LLM. Si el actor de PathCRF cae en un slot sintético, no se puede revertir a un `track_id` real o rompe la posesión esperada, el evento se marca como no listo para comentario y se documenta el `skip_reason`. Para equipos que atacan hacia la derecha, los tercios `[0, L/3]`, `[L/3, 2L/3]` y `[2L/3, L]` son iniciación, creación y finalización; para equipos que atacan hacia la izquierda se intercambian iniciación y finalización.
